...
*/
int syscalls_maxChildrenInRanges(void *ustack)
{
	int x, y;
	int *pid, *count;

	// Take parameter 0 from user and store in x
	GETFROMSTACK(ustack, int, x, 0);
	// Take parameter 1 from user and store in y
	GETFROMSTACK(ustack, int, y, 1);
	// Take parameter 2 from user and store in pid
	GETFROMSTACK(ustack, int *, pid, 2);
	// Take parameter 3 from user and store in count
	GETFROMSTACK(ustack, int *, count, 3);

	return posix_maxChildrenInRanges(x, y, pid, count);
}
