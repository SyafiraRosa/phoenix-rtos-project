...
// Internal helper - must be called with lock already held
static int _posix_countChildren(int pid)
{
    process_info_t *p;
    int count = 0;
    ...
    for (
        node = lib_rbMinimum(posix_common.pid.root);
        node != NULL;
        node = lib_rbNext(node))
    {
        p = lib_treeof(process_info_t, linkage, node);
...
// Wrapper with lock
int posix_countChildren(int pid)
{
    int count;

    proc_lockSet(&posix_common.lock);

    count = _posix_countChildren(pid);
...
int posix_maxChildrenInRanges(int x, int y, int *pid, int *count)
{
	// Check invalid range
	if (x > y)
	{
        ...
        node = lib_rbNext(node))
    {
        ...
        currCount = _posix_countChildren(p->process);

		if (currCount >= x && currCount <= y)
		{
