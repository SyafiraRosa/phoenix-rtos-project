 ...
 * Phoenix-RTOS
 *
 * Caesar Cipher Decoder Server
 *
 * This server decodes plain text using the Caesar cipher with a shift of 5.
...
/* Internal storage for the plain-text */
static char storage_buf[BUF_SIZE];
static size_t storage_len = 0;


/**
 * Apply Caesar cipher shift backward to a single character.
 */
static char shift_char_backward(char character)
{
	if (character >= 'A' && character <= 'Z') {
		/* Upper case shifting backward with alphabetical wrap-around */
		int decoded_index = (character - 'A' - CIPHER_SHIFT + 26) % 26;
		return 'A' + decoded_index;
	}
	else if (character >= 'a' && character <= 'z') {
		/* Lower case shifting backward with alphabetical wrap-around */
		int decoded_index = (character - 'a' - CIPHER_SHIFT + 26) % 26;
		return 'a' + decoded_index;
	}
	else {
...
/**
 * Process a whole stream of characters by applying the backward shift.
 */
static void process_caesar_decode(const char *source, char *destination, size_t len)
{
	for (size_t i = 0; i < len; i++) {
		destination[i] = shift_char_backward(source[i]);
	}
}
...
static ssize_t handle_device_read(oid_t *device_oid, void *user_buffer, size_t requested_len, off_t read_offset)
{
	...
	/* Copy processed plain-text to the requesting user space */
...
static ssize_t handle_device_write(oid_t *device_oid, const void *user_data, size_t data_len, off_t write_offset)
{
	...
	/* Perform decryption and update buffer at specified offset */
	process_caesar_decode((const char *)user_data, storage_buf + write_offset, data_len);
	...
	for (;;) {
		...
		if (status < 0) {
			if (status == -EINTR) {
				...
				fprintf(stderr, "caesar_decode: Critical error in msgRecv: %d (%s)\n",
...
int main(void)
{
	...
	if (portCreate(&server_oid.port) < 0) {
		fprintf(stderr, "caesar_decode: Failed to create IPC port\n");
		return EXIT_FAILURE;
	}

	/* Map the handler to device nodes under /dev */
	if (create_dev(&server_oid, "caesar_decode") < 0) {
		fprintf(stderr, "caesar_decode: Failed to map device in filesystem\n");
		return EXIT_FAILURE;
	}

	printf("caesar_decode: Server is up at /dev/caesar_decode\n");
